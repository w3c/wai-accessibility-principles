...
テキスト代替とは、テキスト以外のコンテンツに相当するものである。例えば、以下のようなものがある：
...
音声を聞いたり映像を見たりすることができない人には、代替手段が必要です。例えば、以下のようなものがある：
...
識別可能なコンテンツは見やすく、聞き取りやすい。そのようなコンテンツには以下のようなものがある：

-   色が情報を伝えたりコンテンツを識別したりする唯一の方法として使用されていない
-   デフォルトの前景色と背景色の組み合わせが十分なコントラストを提供している
-   ユーザーがテキストを400%まで拡大したりテキストの間隔を変更したりしても、情報が失われない
-   テキストが小さなウィンドウ（"ビューポート"）内で再配置され、ユーザーがテキストを大きくしたときも同様である
-   テキストの画像がリサイズ可能であり、実際のテキストに置き換えられるか、可能な限り避けられる
-    ユーザーがウェブサイトで再生される音声を一時停止、停止、または音量を調整できる
-   背景音は低く設定されているか、干渉や気晴らしを避けるためにオフにできる

この要件を満たすことは、前景と背景を区別し、重要な情報をより識別しやすくするのに役立ちます。これには、支援技術を使用しない人々や、背景の目立つ音声や視覚コンテンツから干渉を受ける可能性がある支援技術を使用する人々への配慮が含まれます。例えば、色覚異常の多くの人々は特定のツールを使用せず、テキストとその周囲の背景の間に十分な色のコントラストを提供する適切なデザインに頼っています。他の人にとっては、自動的に再生される音声がテキスト読み上げや[支援聴取デバイス (ALDs)](/teach-advocate/accessible-presentations/#ald "definition")と干渉する可能性があります。
...
この要件を満たすことで、人間工学的なレイアウトのキーボード、画面上のキーボード、スイッチデバイスなどの代替キーボードを使用する人々を含む、キーボードユーザーが支援されます。また、音声認識（音声入力）を使用してウェブサイトを操作したり、キーボードインターフェースを通じてテキストを入力したりする人々にも役立ちます。
...
一部の人々は、テキストを入力したり、指示を理解したり、コントロールを操作したり、ウェブサイトでのタスクを完了するために他の人よりも時間がかかる場合があります。
